...
import {
  EmailAddressesSourceTypeEnum,
  EmailAddressStatusEnum,
  EmailAddressTypeEnum,
  MethodEnum,
} from '../../core/enums/index.js';
import {
  commonEmailAddressDomainsList,
  filterEmailAddresses,
  filterEmailAddressDomains,
  invalidDomains,
...
class CreateEmailService {
  ...
  async initiate(settings) {
    ...
    // Update counts.
    sendEmailService.sendEmailDataModel.updateCount(
      true,
      EmailAddressStatusEnum.PENDING,
      emailDataModel.emailsList.length
    );
    sendEmailService.sendEmailDataModel.updateCount(
      true,
      EmailAddressStatusEnum.TOTAL_PENDING,
      emailDataModel.emailsList.length
    );
    sendEmailService.sendEmailDataModel.updateCount(
      true,
      EmailAddressStatusEnum.DATABASE,
  ...
  async addMonitorEmails(emailDataModel) {
    emailDataModel = await this.getEmailAddressesFile({
      ...
      parameterName: pathUtils.getBasename(
        this.sourceDataModel.monitorFilePath
      ),
      emailDataModel: emailDataModel,
    });
    return emailDataModel;
  ...
  async fetchEmailAddresses(path) {
    ...
    if (
      fileSize > countLimitService.countLimitDataModel.maximumFileSizeMegabytes
    ) {
      switch (this.sourceDataModel.emailAddressesSourceType) {
        case EmailAddressesSourceTypeEnum.DIRECTORY: {
          return [];
        ...
        case EmailAddressesSourceTypeEnum.FILE: {
          throw new Error(
            `File exceeded the maximum size of ${countLimitService.countLimitDataModel.maximumFileSizeMegabytes}MB: ${fileSize}MB (1000011)`
          );
        }
      }
    }
  ...
  async getEmailAddressesDirectory(data) {
    ...
    if (fileUtils.isFilePath(path)) {
      throw new Error(
        `The parameter path ${parameterName} marked as directory but it's a path of a file: ${path} (1000013)`
      );
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (
        fileName.indexOf(this.sourceDataModel.emailAddressesIncludeFileName) ===
        0
      ) {
        // Fetch the email addresses.
  ...
  async getEmailAddressesFile(data) {
    ...
    if (fileUtils.isDirectoryPath(path)) {
      throw new Error(
        `The parameter path ${parameterName} marked as file but it's a path of a directory: ${path} (1000016)`
      );
    }
    // Fetch the email addresses.
  ...
  validateRandomExceeded(emailAddressesList) {
    if (
      emailAddressesList.length <=
      countLimitService.countLimitDataModel.maximumSendEmails
    ) {
      if (!sendEmailService.sendEmailDataModel.totalCount) {
        sendEmailService.sendEmailDataModel.updateCount(
          true,
          EmailAddressStatusEnum.TOTAL,
          emailAddressesList.length
        );
    ...
    if (!sendEmailService.sendEmailDataModel.totalCount) {
      ...
      applicationService.applicationDataModel.method =
        total !== pending ? MethodEnum.RANDOM_EXCEEDED : MethodEnum.STANDARD;
    }
    return emailAddressesList;
  }
  ...
  createEmailData(data) {
    ...
    for (let i = 0; i < emailAddressesList.length; i++) {
      ...
      let emailModel = new EmailModel({
        ...
        type: isMonitorList
          ? EmailAddressTypeEnum.MONITOR
          : EmailAddressTypeEnum.STANDARD,
      ...
      if (isMonitorList) {
        if (
          monitorEmailsCount >=
          countLimitService.countLimitDataModel.monitorEmailsSendCount
        ) {
          break;
        }
        // Pick a remaining element.
        const randomIndex = textUtils.getRandomNumber(
          0,
          emailDataModel.emailsList.length
        );
  ...
  filterEmailAddress(emailModel) {
    ...
    const domainPart = textUtils.getEmailAddressParts(
      emailModel.toEmailAddress
    )[1];
    if (filterEmailAddressDomains.includes(domainPart)) {
      emailModel.status = EmailAddressStatusEnum.FILTER;
      return emailModel;
    }
    const emailAddressIndex = filterEmailAddresses.findIndex(
      (emailAddressItem) =>
        textUtils.toLowerCaseTrim(emailAddressItem) ===
        textUtils.toLowerCaseTrim(emailModel.toEmailAddress)
    );
  ...
  skipDomains(emailDataModel) {
    const maximumUniqueDomainCount =
      countLimitService.countLimitDataModel.maximumUniqueDomainCount;
    if (
      !maximumUniqueDomainCount ||
      emailDataModel.emailsList.length <= maximumUniqueDomainCount
    ) {
      return emailDataModel;
    }
    ...
    for (let i = 0; i < emailDataModel.emailsList.length; i++) {
      ...
      const splitResult = textUtils.getEmailAddressParts(
        emailModel.toEmailAddress
      );
      ...
      if (
        commonEmailAddressDomainsList.findIndex(
          (domain) => domain === domainPart
        ) > -1
      ) {
        updatedEmailDataList.push(emailModel);
        continue;
      }
      const groupIndex = emailDataGroupsList.findIndex(
        (d) => d.domainPart === domainPart
      );
