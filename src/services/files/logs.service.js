...
const { LogsData } = require('../../core/models/application');
...
class LogsService {
	...
	startLogProgress() {
		...
		this.logInterval = setInterval(() => {
			...
			this.logProgress();
		}, countsLimitsService.countsLimitsData.millisecondsIntervalCount);
	}
	...
	async createModeDirectory() {
		this.baseSessionPath = pathUtils.getJoinPath({
			targetPath: pathsService.pathsData.distPath,
	...
	createFilePath(fileName) {
		...
		return pathUtils.getJoinPath({
			targetPath: this.sessionDirectoryPath ? this.sessionDirectoryPath : pathsService.pathsData.distPath,
	...
	getDisplayEmailAddress(emailAddress) {
		return textUtils.cutText({ text: emailAddress, count: countsLimitsService.countsLimitsData.maximumEmailAddressCharactersDisplayCount });
	}
	...
	logProgress() {
		...
		const accountSentPercentage = textUtils.calculatePercentageDisplay({
			...
			totalValue: countsLimitsService.countsLimitsData.maximumSendGridDailyEmailsCount
		});
		const accountSent = textUtils.getNumberOfNumber({
			number1: accountsService.accountData.sentCount,
			number2: countsLimitsService.countsLimitsData.maximumSendGridDailyEmailsCount
		});
		...
		if (sendEmailService.emailData) {
			...
			if (sendEmailService.emailData.resultDetails) {
				resultDetails = textUtils.cutText({ text: sendEmailService.emailData.resultDetails.join(' '), count: countsLimitsService.countsLimitsData.maximumResultCharactersDisplayCount });
			}
		}
