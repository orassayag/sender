...
class LogService {
	...
	initiateDirectories() {
		...
		this.emailResultsPath = this.createFilePath(`email_results_${PlaceholderEnum.DATE}`);
	}
	...
	createModeDirectory() {
		this.baseSessionPath = pathUtils.getJoinPath({
			...
			targetName: textUtils.toLowerCase(applicationService.applicationDataModel.mode)
		});
	...
	getDisplayEmailAddress(emailAddress) {
		return textUtils.cutText({ text: emailAddress, count: countLimitService.countLimitDataModel.maximumEmailAddressCharactersDisplayCount });
	}

	logProgress() {
		const time = `${applicationService.applicationDataModel.time} [${this.frames[this.i = ++this.i % this.frames.length]}]`;
		const currentPercentage = textUtils.calculatePercentageDisplay({ partialValue: applicationService.applicationDataModel.currentEmailIndex, totalValue: sendEmailService.sendEmailDataModel.totalPendingCount });
		const current = textUtils.getNumberOfNumber({ number1: applicationService.applicationDataModel.currentEmailIndex, number2: sendEmailService.sendEmailDataModel.totalPendingCount });
		const currentItem = `${current} (${currentPercentage})`;
		const sentCount = `${StatusIconEnum.V}  ${textUtils.getNumberWithCommas(sendEmailService.sendEmailDataModel.sentCount)}`;
		const errorCount = `${StatusIconEnum.X}  ${textUtils.getNumberWithCommas(sendEmailService.sendEmailDataModel.errorCount)}`;
		const accountSentPercentage = textUtils.calculatePercentageDisplay({
			partialValue: accountService.account.sentCount,
			totalValue: countLimitService.countLimitDataModel.maximumSendGridDailyEmailsCount
		});
		const accountSent = textUtils.getNumberOfNumber({
			number1: accountService.account.sentCount,
			number2: countLimitService.countLimitDataModel.maximumSendGridDailyEmailsCount
		});
		const accountSentItem = `${accountSent} (${accountSentPercentage})`;
		const accounts = textUtils.getNumberOfNumber({ number1: accountService.accountDataModel.currentAccountIndex, number2: accountService.accountDataModel.accountsList.length });
		...
		if (sendEmailService.emailModel) {
			...
			fromEmailAddress = this.getDisplayEmailAddress(sendEmailService.emailModel.fromEmailAddress || accountService.account.username);
			toEmailAddress = this.getDisplayEmailAddress(sendEmailService.emailModel.toEmailAddress);
			id = sendEmailService.emailModel.id;
			type = sendEmailService.emailModel.type;
			if (sendEmailService.emailModel.resultDetails) {
				resultDetails = textUtils.cutText({ text: sendEmailService.emailModel.resultDetails.join(' '), count: countLimitService.countLimitDataModel.maximumResultCharactersDisplayCount });
			}
		}
		logUtils.logProgress({
			...
			ColorEnum.BLUE, ColorEnum.BLUE, ColorEnum.BLUE, ColorEnum.BLUE, ColorEnum.BLUE, ColorEnum.BLUE],
			keysLists: [{
				'Mode': applicationService.applicationDataModel.mode,
				'Method': applicationService.applicationDataModel.method,
				'Database': mongoDatabaseService.mongoDatabaseDataModel.mongoDatabaseModeName,
				...
				'Identical Addresses': sendEmailService.sendEmailDataModel.identicalAddressesCount
			}, {
				'Monitor Sent': sendEmailService.sendEmailDataModel.monitorSentCount,
				'Security Error': sendEmailService.sendEmailDataModel.securityErrorCount,
				'Security Exists': sendEmailService.sendEmailDataModel.securityExistsCount,
				'Missing Field': sendEmailService.sendEmailDataModel.missingFieldCount,
				'Invalid Status': sendEmailService.sendEmailDataModel.invalidStatusCount
			}, {
				'Identical Status': sendEmailService.sendEmailDataModel.identicalStatusCount,
				'Unexpected Field': sendEmailService.sendEmailDataModel.unexpectedFieldCount,
			...
			colorsLists: [
				...
				[ColorEnum.CYAN, ColorEnum.CYAN, ColorEnum.GREEN, ColorEnum.RED, ColorEnum.CYAN, ColorEnum.CYAN],
				[ColorEnum.GREEN, ColorEnum.RED, ColorEnum.CYAN, ColorEnum.CYAN, ColorEnum.CYAN, ColorEnum.RED, ColorEnum.CYAN],
				[ColorEnum.CYAN, ColorEnum.RED, ColorEnum.RED, ColorEnum.RED, ColorEnum.RED],
				[ColorEnum.RED, ColorEnum.RED, ColorEnum.RED, ColorEnum.RED, ColorEnum.RED],
				[ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW],
				[],
				[ColorEnum.YELLOW, ColorEnum.YELLOW],
				[],
				[ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW, ColorEnum.YELLOW],
				[]
			],
	...
	createEmailTemplate(emailModel, isLog) {
		const { id, accountId, accountApiKey, fromEmailAddress, toEmailAddress, subjectId, subjectLine, subjectLineDisplay,
			textId, textLine, textLineDisplay, status, step, type, resultDateTime, resultDetails, resultCode } = emailModel;
	...
	createConfirmSettingsTemplate(settings, mode) {
		const mongoDatabaseModeName = mongoDatabaseService.getMongoDatabaseModeName(settings, mode);
		...
		let settingsText = this.createLineTemplate('DATABASE', mongoDatabaseModeName);
