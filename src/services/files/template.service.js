...
class TemplateService {
  ...
  async initiateTemplates() {
    ...
    for (let i = 0; i < templates.length; i++) {
      ...
      if (subject) {
        this.lastSubjectId++;
        this.templateDataModel.subjectsList.push(
      ...
      if (text) {
        const line = text[0];
        this.lastTextId++;
        this.templateDataModel.textsList.push(
          new TextDataModel({
            id: this.lastTextId,
            text: text.join('\n'),
            textLine: line,
  ...
  async initiateCVFile() {
    ...
    if (extension !== '.doc') {
      throw new Error(
        `The CV file needs to be a doc file. Found a ${extension} file (1000038)`
      );
    }
    this.cvDataModel = new CVDataModel({
      ...
      attachmentBase64: await fileUtils.createBase64Path(
        this.templateDataModel.cvFilePath
      ),
  ...
  getRandomSubject() {
    ...
    if (!this.templateModel?.subjectId) {
      randomSubjectId = textUtils.getRandomNumber(
        1,
        this.templateDataModel.subjectsList.length
      );
    } else {
      for (let i = 0; i < 20; i++) {
        randomSubjectId = textUtils.getRandomNumber(
    ...
    return this.templateDataModel.subjectsList.find(
      (t) => t.id === randomSubjectId
    );
  }
  ...
  getRandomText() {
    ...
    if (!this.templateModel?.textId) {
      randomTextId = textUtils.getRandomNumber(
        1,
        this.templateDataModel.textsList.length
      );
    } else {
      for (let i = 0; i < 20; i++) {
        randomTextId = textUtils.getRandomNumber(
  ...
  getDisplayTemplate(text) {
    ...
    displayTemplate = `${
      validationUtils.isExists(englishKeys) ? `${englishKeys.join(' ')}` : ''
    } ${hebrewKeys.reverse().join(' ')}`.trim();
    if (
      displayTemplate.length >
      countLimitService.countLimitDataModel
        .maximumDisplayTemplateCharactersCount
    ) {
      displayTemplate = displayTemplate.substring(
        0,
        countLimitService.countLimitDataModel
          .maximumDisplayTemplateCharactersCount
      );
