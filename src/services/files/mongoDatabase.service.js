...
import { EmailAddressModel } from '../../core/models/mongo/index.js';
import {
  MongoDatabaseDataModel,
  MongoDatabaseResultModel,
} from '../../core/models/application/index.js';
import { EmailAddressStatusEnum, StatusEnum } from '../../core/enums/index.js';
import countLimitService from './countLimit.service.js';
import globalUtils from '../../utils/files/global.utils.js';
import { systemUtils, textUtils } from '../../utils/index.js';
...
class MongoDatabaseService {
  ...
  async initiate(settings, mode) {
    ...
    this.mongoDatabaseDataModel = new MongoDatabaseDataModel(
      settings,
    ...
    this.mongoConnectionOptions = {
      socketTimeoutMS:
        this.mongoDatabaseDataModel.mongoDatabaseSocketTimeoutMillisecondsCount,
  ...
  async createConnection() {
    ...
    this.client = await mongoose
      .connect(this.mongoConnectionString, this.mongoConnectionOptions)
      .catch((error) => {
        throw new Error(`Failed to connect to MongoDB: ${error} (1000033)`);
      });
    if (!this.client) {
      throw new Error(
        'Failed to connect to MongoDB: Client is null or empty (1000034)'
      );
    }
  }
  ...
  testMongoDatabaseUpdate(result) {
    EmailAddressModel.updateOne(
      { _id: result._id },
      new EmailAddressModel({ emailAddress: 'XXY' })
    );
  }
  ...
  async dropCollection() {
    for (
      let i = 0;
      i < this.mongoDatabaseDataModel.maximumDropCollectionRetriesCount;
      i++
    ) {
      try {
        await this.client.connection
          .collection(this.mongoDatabaseDataModel.mongoDatabaseCollectionName)
          .drop();
        break;
      } catch (error) {}
    }
  ...
  async isEmailAddressExists(emailAddress) {
    ...
    const emailAddressModel = await EmailAddressModel.findOne({
  ...
  async saveEmailAddress(emailAddress) {
    ...
    if (await this.isEmailAddressExists(emailAddress)) {
      return this.setMongoDatabaseErrorResult({
        ...
        description:
          'Email address not saved. Validation of security existence in the Mongo database.',
      });
    }
    let saveResult = null;
    for (
      ...
      countLimitService.countLimitDataModel
        .maximumSaveEmailAddressesRetriesCount;
      i++
    ) {
      try {
        ...
        saveResult = this.setMongoDatabaseErrorResult({
          ...
          description: `Email address not saved. ${systemUtils.getErrorDetails(
            error
          )}`,
        });
      }
  ...
  getErrorInARowResult(isSave, saveError) {
    ...
    return this.saveErrorInARowCount >=
      countLimitService.countLimitDataModel.maximumSaveErrorInARowCount
      ? StatusEnum.SAVE_ERROR_IN_A_ROW
      : null;
  }
  ...
  async simulate() {
    // Simulate result.
    let saveResult = null;
    const isSave = textUtils.getRandomBooleanByPercentage(
      countLimitService.countLimitDataModel.simulateSaveSuccessPercentage
    );
    if (isSave) {
      saveResult = this.setMongoDatabaseSaveResult({
        status: EmailAddressStatusEnum.SAVE,
    ...
    await globalUtils.sleep(
      countLimitService.countLimitDataModel
        .millisecondsSimulateDelaySaveProcessCount
    );
    return saveResult;
  }
